...
def parser():
    ...
    parser.add_argument("--batch_size", default=1, type=int,
                        help=("number of images to be processed at "
                              "the same time"))
    ...
    parser.add_argument("--save_labels", action='store_true',
                        help=("save detections bbox for each image in"
                              "yolo format"))
...
def batch_detection(network, images, class_names, class_colors,
    ...
    batch_detections = darknet.network_predict_batch(
        network, darknet_images, batch_size, image_width,
...
def save_annotations(name, image, detections, class_names):
    ...
    with open(file_name, "w") as f:
        for label, confidence, bbox in detections:
            ...
            f.write(f"{label} {x:.4f} {y:.4f} "
                    f"{w:.4f} {h:.4f} {float(confidence):.4f}")
    ...
    while True:
        ...
        if not args.dont_show:
            ...
            if cv2.waitKey() & 0xFF == ord('q'):
                break
        if args.write_image:
            cv2.imwrite(f"val_{image_name}", image)
        index += 1
