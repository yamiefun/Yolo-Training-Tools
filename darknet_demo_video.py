...
def parser():
    ...
    parser.add_argument("--thresh", type=float, default=.25,
                        help=("remove detections with confidence"
                              "below this value"))
...
def check_arguments_errors(args):
    ...
    if str2int(args.input) == str and not os.path.exists(args.input):
        raise(ValueError(
            f"Invalid video path {os.path.abspath(args.input)}"))
...
def convert4cropping(image, bbox):
    ...
    orig_right = min(orig_right, image_w - 1)
    orig_top = max(orig_top, 0)
    orig_bottom = min(orig_bottom, image_h - 1)
...
def drawing(frame_queue, detections_queue, fps_queue):
    ...
    video = set_saved_video(
        cap, args.out_filename, (video_width, video_height))
    while cap.isOpened():
